...
function sharedWriteRule(hostname, topLevel){       
    ...
    var to = "^https://(www\\.)?" + title.toLowerCase() + "\\"  + topLevel;
    var rule;
    var versionTag = "Rule generated by HTTPS Finder" + strings.getString("httpsfinder.version");
    XML.ignoreComments = false;

    //Check hostname for "www.".
    //One will be "domain.com" and the other will be "www.domain.com"
    var domains = hostname.split(".");
    if(domains.length > 1 && hostname.indexOf("www.") == -1){
        //Then the hostname contains a non-www subdomain. Don't touch it.
        ...
        var host =  this.getHostWithoutSub(hostname);
        rule = <{'ruleset'} name = {title}>
        <{'target'} host={host}/>
        <{'target'} host={hostname}/>
        <{'rule'} from={from} to={to}/>
